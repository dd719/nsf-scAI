...
def mkdir_p(pth): #创建文件夹目录
  pathlib.Path(pth).mkdir(parents=True,exist_ok=True)

def rm_glob(pth,glob="*"): #删除制定目录下制定后缀名的文件
  """
...
def poisson_loss(y,mu): #似然损失
    ...
    with np.errstate(divide='ignore',invalid='ignore'):
        res = mu-y*np.log(mu)
    return np.mean(res[np.isfinite(res)]) #计算非无穷值的平均值作为loss

def poisson_deviance(y,mu,agg="sum",axis=None): #泊松偏差损失
    """
...
def make_grid(N,xmin=-2,xmax=2,dtype="float32"): #生成网格
  x = np.linspace(xmin,xmax,num=int(np.sqrt(N)),dtype=dtype)
  return np.stack([X.ravel() for X in np.meshgrid(x,x)],axis=1)

def kmeans_inducing_pts(X,M): #X：输入数据，形状为 (N, D)，其中 N 是样本数，D 是特征数
  M = int(M)
  Z = np.unique(X, axis=0) #去除 X 中重复的行，返回唯一的数据点
  unique_locs = Z.shape[0]
  if M<unique_locs:
    Z=KMeans(n_clusters=M).fit(X).cluster_centers_
  return Z

def lnormal_approx_dirichlet(L): #通过独立的对数正态分布近似对称平坦狄利克雷分布
  """
